...
def main():
    ...
    if objective == 'worst' and CONFIG['plugin_params']['use_eg_outer']:
        ...
        alpha_star, mu_star, t_group_star, beta_star, eg_hist = worst_group_eg_outer(
            ...
            alpha_init=alpha_init,  # Pass selective alpha init
            mu_init=mu_init,  # Pass selective mu init
            freeze_alpha=CONFIG['plugin_params'].get('freeze_alpha', False),  # Pass freeze flag
            freeze_mu=CONFIG['plugin_params'].get('freeze_mu', False)  # Pass freeze_mu flag
        )
        
        # Use the per-group thresholds directly from EG-outer
        ...
        t_star = t_group[0]  # Use first threshold as global for compatibility
        ...
        if selective_loaded and init_t is not None:
            # Handle both tensor and scalar init_t
            if isinstance(init_t, torch.Tensor):
                init_t_val = init_t.item() if init_t.numel() == 1 else init_t.tolist()
            else:
                init_t_val = init_t
            print(f"ℹ️ Using selective raw-margin threshold init t={init_t_val} (will refit per plugin iteration)")
